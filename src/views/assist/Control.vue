    ...
    <el-card class="box-card" style="margin-top: 20px; height: 460px">
      ...
      >
        ...
        >
          <div>
            ...
            <el-button
              ...
              icon="el-icon-delete"
              style="float: right"
              @click="handleDelete(item.ta_message_id)"
              circle
            ></el-button>
          </div>
        </el-collapse-item>
        <el-collapse-item
          ...
          style="color: #cc0033"
          v-if="item.is_read == 0"
          @click.native="isRead(item.ta_message_id)"
        >
...
export default {
  ...
  methods: {
      ...
      this.axios
        ...
        .then(
          (response) => {
            ...
            if (response.data.status === 400) this.$message("找不到助教");
            else {
    ...
    isRead(ta_message_id) {
      var json = {
        ta_id: this.ta_id,
        ta_message_id: ta_message_id,
      };
