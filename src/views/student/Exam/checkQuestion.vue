...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.axios.post("/api/submitExam",JSON.stringify({
    ...
    timer() {//定义计时函数
      ...
      this.time.timeStr = this.toDub(this.time.hour) + ':' + this.toDub(this.time.minute) + ':' + this.toDub(this.time.second)/*+""+this.toDubms(this.ms)+"毫秒"*/
