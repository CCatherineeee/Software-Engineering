      ...
      <div v-for="(item,index) in questionList" :key="index">
        <el-card>
          <div>
            ...
            <el-form-item v-if="item.q_type === 1" :class="getClass(index,item.q_type)">
              <br />

              <el-radio-group disabled v-model="answerList[index].answer">
                <el-radio label= "1">A. {{ item.option_a }}
                  <br />
                  <br />
                </el-radio >
                <br />
                <el-radio label= "2">B. {{ item.option_b }}
                  <br />
                  <br />
                </el-radio>
                <br />
                <el-radio label= "3">C. {{ item.option_c }}
                  <br />
                  <br />
                </el-radio>
                <br />
                <el-radio label= "4">D. {{ item.option_d }}
                  <br />
                  <br />
                </el-radio>
              </el-radio-group>
            ...
            <el-form-item v-if="item.q_type === 2" :class="getClass(index,item.q_type)">

              <el-checkbox-group v-model="answerList[index].checkList" disabled>
            ...
            <el-form-item v-if="item.q_type === 3" :class="getClass(index,item.q_type)" >
              <el-radio-group disabled v-model="answerList[index].answer">
  ...
  methods:{
    ...
    getExam(){
      this.axios.post('/api/getCloseExam',JSON.stringify({
        ...
        if(res.data.code ===200){
          ...
          if(res.data.data.answerList === null)
            this.answerList = []
          else
          ...
          for(let i = 0; i< this.questionList.length; i++){
            if(this.questionList[i].q_type === 1||this.questionList[i].q_type === 3){
              ...
              let t = [];
              for(let j =0 ;j <this.questionList[i].checkList.length;j++){
    ...
    getClass(index,type){
      if(index > this.answerList.length)
        return "color-red"
      if(type === 1 || type === 3){
        if(this.questionList[index].answer.toString() === this.answerList[index].answer)
          return "color-green"
        else
          return "color-red"
      }
      else{
        if(this.questionList[index].checkList === this.answerList[index].checkList)
          return "color-green"
        else
          return "color-red"
      }
