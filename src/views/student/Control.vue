<template>
  <div >
    ...
    <el-card class="box-card" style="margin-top:20px;height:460px" >
        ...
        v-for="item in msgList.slice((currentPage-1)*pagesize,currentPage*pagesize)"
        :key="item.stu_message_id"
        accordion>
          <el-collapse-item :title="item.create_time+item.title" :name="item.seq" style="color:#666666"  v-if="item.is_read == 1" >
            <div>
              {{item.content}}
            <el-button type="danger" icon="el-icon-delete" style="float:right;" @click="deleteMsg(item.stu_message_id)" circle></el-button>
            </div>
          </el-collapse-item>
          <el-collapse-item :title="item.create_time+item.title" :name="item.seq" style="color:#CC0033"  v-if="item.is_read == 0"  @click.native="isRead(item.stu_message_id)">
