...
def addClassFile():
    ...
    if not this_class:  #班级不存在
        return jsonify({'status':500,'message':'班级不存在','data':None})
    ...
    if not ClassFile.query.filter(ClassFile.file_url == filepath).first():
        ...
        result = {'status':200,'message':'上传成功','data':None}
...
def deleteClassFile():
    ...
    if class_file:
        ...
        dbManage.db.session.commit()
        result = {'status':200,'message':'删除成功','data':None}
    else:
        result = {'status':400,'message':'删除失败','data':None}
